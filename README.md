...
当前发布版本：`v1.16`，包含成员搜索、父子需求创建及界面一致性改进。生产升级需要在服务器执行升级脚本并通过健康检查。

## v1.16 视频与动图附件

- 新建需求、需求详情、评论均支持 JPG、PNG、WebP、GIF、MP4、WebM。视频单文件最大 100 MB，GIF 最大 20 MB，其他图片保持 500 MB；服务端同时遵守更低的 Storage.MaxAttachmentBytes 配置上限。
- 视频列表显示播放入口，点击后鉴权读取文件并打开播放器，可暂停、拖动进度、调整音量和全屏，不自动播放。GIF 使用原文件展示动画。浏览器不支持的视频编码会提示下载原文件；本轮不提供转码。
- 上传沿用分片与进度反馈，服务端校验扩展名、声明类型和容器文件头；旧附件不迁移、不删除，无数据库结构变更。
- 待上传附件可移除；已上传普通附件仅上传人或管理员可删除。评论作者或管理员可以确认删除整条评论及其附件，评论附件不单独删除；尚未发出的提及通知取消，已发出的通知不撤回。
- 本地预览数据与生产隔离，升级沿用备份并保留生产数据的发布流程。
